...
public class INDProfiler {
    ...
    public List<IND> profile(List<Relation> relations, boolean discoverNary) {
        ...
        for (Relation r1 : relations) {
            String[][] columns1 = r1.getColumns();
            for (int i = 0; i < columns1.length; i++) {
                Set<String> sourceValues = new HashSet<>(Arrays.asList(columns1[i]));
                for (Relation r2 : relations) {
                    String[][] columns2 = r2.getColumns();
                    for (int j = 0; j < columns2.length; j++) {
                        if (r1 == r2 && i == j)
                            continue;
                        Set<String> targetValues = new HashSet<>(Arrays.asList(columns2[j]));
                        if (targetValues.containsAll(sourceValues)) {
                            inclusionDependencies.add(new IND(r1, i, r2, j));
                        }
                    }
                }
            }
